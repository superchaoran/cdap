...
import co.cask.cdap.common.lang.ExtensionClassInstantiator;
...
public class CustomLogProcessors {
  ...
  public List<LogProcessor> getLogProcessors() throws Exception {
    ...
    if (!jarPath.isEmpty()) {
      logProcessors.add(getLogProcessor(jarPath));
    }
    return logProcessors;
  }

  /**
   * check if user has configured a list of custom log processors.
   * @return
   */
  public boolean hasLogProcessors() {
    return !jarPath.isEmpty();
  }
  ...
  private LogProcessor getLogProcessor(String logProcessorPluginJarPath) throws Exception {
    try {
      ...
      ExtensionClassInstantiator.ensureValidExtensionJar(logProcessorExtenstionJar, "LogProcessor",
  ...
  public Properties getLogProcessorExtensionProperties() {
    return ExtensionClassInstantiator.createExtensionProperties(cConf, Constants.LogSaver.EXTENSION_CONFIG_PREFIX);
  }
  ...
  private LogProcessor createAndInitializeLogProcessor(File logProcessorJar,
    ...
    Class<? extends LogProcessor> logProcessorClass =
      ExtensionClassInstantiator.loadExtensionClass(logProcessorJar, logProcessorClassLoader,
