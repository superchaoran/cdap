...
public class KafkaLogWriterPlugin extends AbstractKafkaLogProcessor {
  ...
  KafkaLogWriterPlugin(CConfiguration cConf, FileMetaDataManager fileMetaDataManager,
    ...
    if (!customLogProcessors.hasLogProcessors()) {
      logProcessorExtensions = new ArrayList<>();
    } else {
      logProcessorExtensions = customLogProcessors.getLogProcessors();
    }

    logProcessorExtensionProperties = customLogProcessors.getLogProcessorExtensionProperties();
